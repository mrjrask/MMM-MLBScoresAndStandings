...
module.exports = NodeHelper.create({
  ...
  fetchStandingsFromFile() {
    const filePath = path.join(__dirname, "standings.json");
    try {
      const data    = fs.readFileSync(filePath, "utf8");
      const records = JSON.parse(data);
      console.log(
        "[MMM-MLBScoresAndStandings] loaded standings from file:",
        records.length
      );
      this.sendSocketNotification("STANDINGS", records);
    } catch (e) {
      console.error(
        "[MMM-MLBScoresAndStandings] read standings file error",
        e
      );
