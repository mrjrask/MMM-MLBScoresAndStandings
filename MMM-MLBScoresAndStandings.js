...
if (typeof Module !== "undefined" && Module.register) {
  Module.register("MMM-MLBScoresAndStandings", {
    ...
    socketNotificationReceived(notification, payload) {
      console.log("[MMM-MLBScoresAndStandings] received", notification, "len=", payload.length);
    ...
    getDom() {
      console.log("[MMM-MLBScoresAndStandings] getDom mode=", this.rotationMode, "games=", this.games.length, "divs=", this.divisions.length);
      ...
      if (this.rotationMode === "games" && this.games.length === 0) {
        const msg = document.createElement("div"); msg.innerText = "No games to display.";
        wrapper.appendChild(msg);
        return wrapper;
      }
      if (this.rotationMode === "standings" && this.divisions.length === 0) {
        const msg = document.createElement("div"); msg.innerText = "Standings unavailable.";
        wrapper.appendChild(msg);
        return wrapper;
      }
      ...
      if (this.rotationMode === "games") {
        ...
        const colWrapper = document.createElement("div"); colWrapper.className = "games-columns";
    ...
    createGameBox(game) {
      ...
      // Header row: blank, timeCell, R, H, E
      const trHdr = document.createElement("tr");
      // first blank
      let th = document.createElement("th"); th.innerText = ""; trHdr.appendChild(th);
      // time header blank
      th = document.createElement("th"); th.innerText = ""; trHdr.appendChild(th);
      ["R","H","E"].forEach(lbl => {
      ...
      [game.teams.away, game.teams.home].forEach(teamData => {
        ...
        // Status cell (time/inning/final)
        td = document.createElement("td");
        td.className = "status-cell";
        td.innerText = timeText;
        tr.appendChild(td);
