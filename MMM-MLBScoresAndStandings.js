...
Module.register("MMM-MLBScoresAndStandings", {
  ...
  _buildStandings() {
    ...
    slice.forEach(divId => {
      ...
      if (grp) {
        ...
        h3.innerText = DIVISION_LABELS[divId]; h3.style.margin = "0 0 4px 0";
        div.appendChild(h3);
  ...
  createGameBox(game) {
    ...
    [game.teams.away, game.teams.home].forEach((t,i) => {
      ...
      const sp = document.createElement("span"); sp.className = "abbr"; sp.innerText = abbr; if(isFin) sp.classList.add("final"); tdT.appendChild(sp); tr.appendChild(tdT);
  ...
  createStandingsTable(group) {
    ...
    group.teamRecords.forEach(rec => {
      ...
      const sp2 = document.createElement("span"); sp2.className="abbr"; sp2.innerText=ab; tdTeam.appendChild(sp2); tr.appendChild(tdTeam);
      ...
      let gb=rec.divisionGamesBack; if(gb!=null&&gb!="-"){const f=parseFloat(gb),m=Math.floor(f),r=f-m;
        if(Math.abs(r)<1e-6) gb=`${m}`; else if(r===0.5) gb=m===0?"½":`${m}½`; else gb=f.toString(); }
